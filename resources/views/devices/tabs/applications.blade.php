@php
    ...
    $chartColors = ['#3B82F6', '#10B981', '#8B5CF6', '#F59E0B', '#EF4444', '#EC4899', '#06B6D4', '#84CC16', '#F97316', '#14B8A6'];
@endphp
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
    ...
    <div class="glass-card rounded-xl overflow-hidden">
        ...
        <div class="p-4 max-h-80 overflow-y-auto">
            ...
            @endphp
            <div class="flex items-center justify-between py-2.5 {{ $index > 0 ? 'border-t border-purple-500/10' : '' }} hover:bg-purple-500/10 px-2 rounded transition-colors">
                ...
                <div class="flex items-center gap-4">
                    <div class="w-24 h-2 bg-purple-500/10 rounded-full overflow-hidden">
                        <div class="h-full rounded-full" style="width: {{ $percent }}%; background-color: {{ $chartColors[$index % count($chartColors)] }}"></div>
                    </div>
                    <span class="text-xs text-purple-300/60 w-12 text-right">{{ $percent }}%</span>
    ...
    <div class="p-4">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="space-y-3">
                ...
                @endphp
                <div class="flex items-center justify-between p-3 bg-purple-500/10 rounded-lg hover:bg-purple-500/15 transition-colors">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-purple-300/50">{{ number_format($cloud['unique_ips']) }} unique IPs</p>
                        </div>
                    </div>
                    <div class="text-right">
                        ...
                        <p class="text-xs text-purple-300/50">{{ $percent }}% of cloud traffic</p>
                    </div>
                </div>
                @endforeach
            </div>
        ...
        <div class="flex flex-col items-center justify-center py-12 text-purple-300/50">
            <svg class="w-16 h-16 mb-3 text-purple-400/30" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 15a4 4 0 004 4h9a5 5 0 10-.1-9.999 5.002 5.002 0 10-9.78 2.096A4.001 4.001 0 003 15z"/>
            </svg>
            <p class="text-sm font-medium mb-1">No Cloud Traffic Detected</p>
            <p class="text-xs text-purple-400/40">Cloud service traffic will appear here when detected</p>
        </div>
        @endif
    </div>
</div>
                ...
                pie: {
                    donut: {
                        ...
                        labels: {
                            ...
                            name: { color: '#a78bfa' },
                            value: { color: '#fff', formatter: (val) => formatBytes(parseInt(val)) },
                            total: { show: true, label: 'Cloud Traffic', color: '#a78bfa', formatter: (w) => formatBytes(w.globals.seriesTotals.reduce((a, b) => a + b, 0)) }
                        }
                    }
                }
            },
