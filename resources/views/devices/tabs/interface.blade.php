...
<div class="space-y-4">
    @if($device->interfaces->isEmpty() && $interfaceTraffic->isEmpty())
        ...
        <div class="bg-white rounded-xl shadow-lg p-8 text-center">
            ...
            <h3 class="text-lg font-bold text-gray-900 mb-2">No Interface Data Available</h3>
            <p class="text-gray-600 mb-4">Interface data will appear once NetFlow data with interface information is received.</p>
            <div class="bg-[#E4F2FF] rounded-lg p-4 text-left max-w-md mx-auto">
                <p class="text-sm text-[#5548F5] font-medium mb-2">To enable interface tracking:</p>
                <ul class="text-sm text-gray-600 space-y-1">
                    <li class="flex items-start gap-2">
                        <span class="text-[#5548F5]">•</span>
    ...
    @else
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
            <div class="bg-white rounded-xl shadow p-4">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-xs text-gray-500 font-medium">Total Interfaces</p>
            ...
            <div class="bg-white rounded-xl shadow p-4">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-xs text-gray-500 font-medium">Active (with traffic)</p>
        ...
        <div class="bg-white rounded-xl shadow p-5 hover:shadow-md transition border-l-4 border-[#5548F5]">
            <div class="flex justify-between items-start mb-3">
                <div>
                    <h4 class="text-base font-bold text-gray-900">{{ $interface->name }}</h4>
                    @if($interface->description)
                    <p class="text-sm text-gray-500">{{ $interface->description }}</p>
                    @endif
                </div>
                <span class="px-2.5 py-1 text-xs font-semibold rounded-full {{ $interface->status === 'up' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                <div class="bg-gray-50 rounded-lg p-2.5">
                    ...
                    <p class="font-medium text-gray-900">{{ ucfirst($interface->type ?? 'Unknown') }}</p>
                </div>
                <div class="bg-gray-50 rounded-lg p-2.5">
                    <p class="text-xs text-gray-500">Speed</p>
                    <p class="font-medium text-gray-900">{{ $interface->formatted_speed ?? 'N/A' }}</p>
                </div>
            ...
            <div class="mt-3">
                <div class="flex justify-between items-center mb-1">
                    ...
                    <span class="text-xs font-semibold text-gray-900">{{ number_format($interface->utilization_percent, 1) }}%</span>
                </div>
        ...
        @empty
            ...
            <div class="bg-white rounded-xl shadow overflow-hidden">
                <div class="px-5 py-3 bg-gradient-to-r from-[#5548F5] to-[#C843F3]">
                    ...
                    <div class="p-4 hover:bg-gray-50 transition">
                        <div class="flex justify-between items-center">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <p class="font-medium text-gray-900">Interface {{ $ifIndex }}</p>
                                    <p class="text-xs text-gray-500">{{ number_format($traffic->flow_count) }} flows</p>
                                </div>
                            </div>
                            <div class="text-right">
                                ...
                                <p class="text-xs text-gray-500">{{ number_format($traffic->in_packets) }} packets</p>
                            </div>
                        </div>
                    </div>
                    @endforeach
