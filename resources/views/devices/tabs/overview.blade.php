...
<div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
    <div class="glass-card rounded-xl p-5 border-l-4 border-blue-500">
        <div class="flex items-center justify-between">
            <div>
                <p class="text-xs text-purple-300/70 font-medium uppercase tracking-wider mb-1">Total Flows</p>
    ...
    <div class="glass-card rounded-xl p-5 border-l-4 border-purple-500">
        <div class="flex items-center justify-between">
            <div>
                <p class="text-xs text-purple-300/70 font-medium uppercase tracking-wider mb-1">Total Traffic</p>
    ...
    <div class="glass-card rounded-xl p-5 border-l-4 border-cyan-500">
        <div class="flex items-center justify-between">
            <div>
                <p class="text-xs text-purple-300/70 font-medium uppercase tracking-wider mb-1">Total Packets</p>
    ...
    <div class="glass-card rounded-xl p-5 border-l-4 border-amber-500">
        <div class="flex items-center justify-between">
            <div>
                <p class="text-xs text-purple-300/70 font-medium uppercase tracking-wider mb-1">Avg Bandwidth</p>
                <p class="text-2xl font-bold text-white">
                    @php
            ...
            <div class="mt-3 grid grid-cols-2 gap-3">
                <div class="bg-blue-500/10 rounded-lg p-3 border border-blue-500/20">
                    <div class="flex items-center gap-1.5 mb-1">
                        ...
                        <p class="text-[10px] text-purple-300/70 font-medium uppercase">Inbound</p>
                    </div>
                    ...
                    <p class="text-[10px] text-purple-300/50">{{ $trafficDistribution['inbound_percent'] ?? 0 }}%</p>
                </div>
                <div class="bg-emerald-500/10 rounded-lg p-3 border border-emerald-500/20">
                    <div class="flex items-center gap-1.5 mb-1">
                        ...
                        <p class="text-[10px] text-purple-300/70 font-medium uppercase">Outbound</p>
                    </div>
                    ...
                    <p class="text-[10px] text-purple-300/50">{{ $trafficDistribution['outbound_percent'] ?? 0 }}%</p>
                </div>
            </div>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const chartColors = ['#3B82F6', '#10B981', '#8B5CF6', '#F59E0B', '#EF4444', '#EC4899', '#06B6D4', '#84CC16'];
                ...
                pie: {
                    donut: {
                        ...
                        labels: {
                            ...
                            name: { color: '#a78bfa' },
                            value: { color: '#fff', formatter: (val) => formatBytes(parseInt(val)) },
                            total: { show: true, color: '#a78bfa', formatter: (w) => formatBytes(w.globals.seriesTotals.reduce((a, b) => a + b, 0)) }
                        }
                    }
                }
            },
    ...
    if (protocolData && protocolData.length > 0) {
        new ApexCharts(document.querySelector("#protocolBarChart"), {
            chart: { type: 'bar', height: 280, background: 'transparent', foreColor: '#a78bfa', toolbar: { show: false } },
            series: [{ name: 'Traffic', data: protocolData.map(p => parseInt(p.total_bytes)) }],
            xaxis: {
                categories: protocolData.map(p => p.protocol),
                labels: { style: { colors: '#a78bfa', fontSize: '10px' } },
                axisBorder: { show: false },
                axisTicks: { show: false }
            },
            yaxis: { labels: { formatter: formatBytes, style: { colors: '#a78bfa', fontSize: '10px' } } },
            colors: chartColors,
            plotOptions: { bar: { borderRadius: 6, columnWidth: '70%', distributed: true } },
            legend: { show: false },
            dataLabels: { enabled: false },
            grid: { borderColor: 'rgba(139, 92, 246, 0.1)' },
            tooltip: { theme: 'dark', y: { formatter: formatBytes } }
        }).render();
    }
    ...
    if (timeSeriesData.labels && timeSeriesData.labels.length > 0) {
        new ApexCharts(document.querySelector("#trafficTimeChart"), {
            chart: { type: 'area', height: 280, background: 'transparent', foreColor: '#a78bfa', toolbar: { show: true } },
            series: [
            ...
            xaxis: {
                ...
                labels: { style: { colors: '#a78bfa', fontSize: '10px' }, rotate: -45 },
                axisBorder: { show: false },
                axisTicks: { show: false }
            },
            yaxis: [
                { labels: { formatter: formatBytes, style: { colors: '#3B82F6' } } },
                { opposite: true, labels: { formatter: (v) => v >= 1000 ? (v/1000).toFixed(0) + 'K' : v, style: { colors: '#10B981' } } }
            ],
            colors: ['#3B82F6', '#10B981'],
            fill: { type: 'gradient', gradient: { opacityFrom: 0.5, opacityTo: 0.1 } },
            stroke: { curve: 'smooth', width: [2, 2] },
            legend: { position: 'top', horizontalAlign: 'right', fontSize: '11px', labels: { colors: '#a78bfa' } },
            grid: { borderColor: 'rgba(139, 92, 246, 0.1)' },
            tooltip: { theme: 'dark', shared: true }
        }).render();
    } else {
        document.getElementById('trafficTimeChart').innerHTML = '<div class="flex items-center justify-center h-full text-purple-300/50"><p>No traffic data for this time range</p></div>';
    }
});
</script>
@endpush
