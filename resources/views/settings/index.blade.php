...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    @if(session('success'))
        <div class="glass-card bg-green-500/20 border-green-500/30 text-green-300 px-6 py-4 rounded-xl mb-6 flex items-center gap-3">
    ...
    <form method="POST" action="{{ route('settings.update') }}">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="p-6">
                        ...
                        <div class="mt-6 pt-6 border-t border-white/10">
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                                <label class="flex items-center p-4 bg-purple-500/10 rounded-xl cursor-pointer hover:bg-purple-500/20 transition border border-purple-500/20 has-[:checked]:border-purple-500">
                                    <input type="checkbox" name="netflow_v5" value="1"
                                           {{ old('netflow_v5', $settings['netflow_v5'] ?? true) ? 'checked' : '' }}
                                           class="rounded border-gray-600 text-purple-500 focus:ring-purple-500 bg-white/10">
                                    <span class="ml-3">
                                ...
                                <label class="flex items-center p-4 bg-pink-500/10 rounded-xl cursor-pointer hover:bg-pink-500/20 transition border border-pink-500/20 has-[:checked]:border-pink-500">
                                    <input type="checkbox" name="ipfix" value="1"
                                           {{ old('ipfix', $settings['ipfix'] ?? true) ? 'checked' : '' }}
                                           class="rounded border-gray-600 text-pink-500 focus:ring-pink-500 bg-white/10">
                                    <span class="ml-3">
                                        <span class="block text-sm font-semibold text-white">IPFIX</span>
                                        <span class="block text-xs text-gray-400">v10 Standard</span>
                                    </span>
                                </label>
                                <label class="flex items-center p-4 bg-fuchsia-500/10 rounded-xl cursor-pointer hover:bg-fuchsia-500/20 transition border border-fuchsia-500/20 has-[:checked]:border-fuchsia-500">
                                    <input type="checkbox" name="sflow" value="1"
                                           {{ old('sflow', $settings['sflow'] ?? false) ? 'checked' : '' }}
                                           class="rounded border-gray-600 text-fuchsia-500 focus:ring-fuchsia-500 bg-white/10">
                                    <span class="ml-3">
                        ...
                        <div class="mt-6 pt-6 border-t border-white/10 space-y-4">
                            <label class="flex items-center p-4 bg-white/5 rounded-xl cursor-pointer hover:bg-white/10 transition">
                                <input type="checkbox" name="aggregation_enabled" value="1"
                                       ...
                                       class="rounded border-gray-600 text-purple-500 focus:ring-purple-500 w-5 h-5 bg-white/10">
                                <span class="ml-4">
                ...
                <div class="glass-card overflow-hidden">
                    <div class="px-6 py-5 border-b border-white/10 bg-gradient-to-r from-orange-500/20 to-red-500/20">
                        <h3 class="text-lg font-bold text-white flex items-center gap-3">
                        ...
                        <p class="text-gray-400 text-sm mt-2 ml-13">Configure when to trigger alerts</p>
                    </div>
                    ...
                    <div class="p-6">
                        ...
                        <div class="space-y-3">
                            <div class="bg-purple-500/20 rounded-xl p-4 border border-purple-500/30">
                                ...
                                <p class="text-2xl font-mono font-bold text-purple-300 mt-1">{{ $settings['collector_ip'] ?: 'Not Configured' }}</p>
                            </div>
                            <div class="bg-pink-500/20 rounded-xl p-4 border border-pink-500/30">
                                <p class="text-xs text-gray-400 uppercase tracking-wide font-semibold">NetFlow Port (UDP)</p>
                                <p class="text-2xl font-mono font-bold text-pink-300 mt-1">{{ $settings['netflow_port'] ?: 'Not Configured' }}</p>
                            </div>
                ...
                <div class="glass-card overflow-hidden">
                    ...
                    <div class="p-6" x-data="{ activeTab: 'cisco' }">
                        <div class="flex flex-wrap gap-2 mb-4">
                            <button type="button" @click="activeTab = 'cisco'" :class="activeTab === 'cisco' ? 'bg-purple-500 text-white shadow-lg' : 'bg-white/10 text-gray-300 hover:bg-white/20'"
                                    class="px-4 py-2 text-xs font-semibold rounded-lg transition">Cisco IOS</button>
                            <button type="button" @click="activeTab = 'fortigate'" :class="activeTab === 'fortigate' ? 'bg-purple-500 text-white shadow-lg' : 'bg-white/10 text-gray-300 hover:bg-white/20'"
                                    class="px-4 py-2 text-xs font-semibold rounded-lg transition">FortiGate</button>
                            <button type="button" @click="activeTab = 'paloalto'" :class="activeTab === 'paloalto' ? 'bg-purple-500 text-white shadow-lg' : 'bg-white/10 text-gray-300 hover:bg-white/20'"
                                    class="px-4 py-2 text-xs font-semibold rounded-lg transition">Palo Alto</button>
                            <button type="button" @click="activeTab = 'juniper'" :class="activeTab === 'juniper' ? 'bg-purple-500 text-white shadow-lg' : 'bg-white/10 text-gray-300 hover:bg-white/20'"
                        ...
                        <div x-show="activeTab === 'cisco'">
                            <pre class="bg-gray-900/80 text-green-400 p-4 rounded-xl text-xs overflow-x-auto max-h-48 border border-white/10">flow exporter MONETX-EXPORT
...
interface GigabitEthernet0/0
 ...
 ip flow monitor MONETX-MONITOR output</pre>
                        ...
                        <div x-show="activeTab === 'fortigate'" style="display: none;">
                            <pre class="bg-gray-900/80 text-green-400 p-4 rounded-xl text-xs overflow-x-auto max-h-48 border border-white/10">config system netflow
...
config system interface
    ...
    next
                        ...
                        <div x-show="activeTab === 'paloalto'" style="display: none;">
                            <pre class="bg-gray-900/80 text-green-400 p-4 rounded-xl text-xs overflow-x-auto max-h-48 border border-white/10">set deviceconfig system netflow
...
commit</pre>
                        ...
                        <div x-show="activeTab === 'juniper'" style="display: none;">
                            <pre class="bg-gray-900/80 text-green-400 p-4 rounded-xl text-xs overflow-x-auto max-h-48 border border-white/10">set services flow-monitoring version-ipfix
    template ipv4
...
set forwarding-options sampling instance sample
    ...
    version-ipfix template ipv4</pre>
                        ...
                        @else
                        <div class="bg-orange-500/20 border border-orange-500/30 rounded-xl p-4 text-center">
                            ...
                            <p class="text-sm font-medium text-orange-300">Configure Collector IP and NetFlow Port first</p>
                            <p class="text-xs text-orange-400 mt-1">Device configuration examples will appear after saving settings</p>
                        </div>
                        @endif
                    </div>
                </div>
                ...
                <div class="glass-card overflow-hidden">
                    ...
                    <div class="p-6 space-y-4">
                        <div class="flex items-center justify-between p-4 bg-green-500/10 rounded-xl border border-green-500/20">
                            ...
                            <span class="px-3 py-1.5 text-xs font-semibold bg-green-500/20 text-green-400 rounded-full border border-green-500/30">Running</span>
                        </div>
                        ...
                        <div class="flex items-center justify-between p-4 bg-green-500/10 rounded-xl border border-green-500/20">
                            ...
                            <span class="px-3 py-1.5 text-xs font-semibold bg-green-500/20 text-green-400 rounded-full border border-green-500/30">Active</span>
                        </div>
                        ...
                        <div class="mt-4 pt-4 border-t border-white/10">
                            <div class="grid grid-cols-2 gap-3 text-center">
                                <div class="bg-purple-500/20 rounded-xl p-4 border border-purple-500/30">
                                    ...
                                    <p class="text-lg font-bold text-purple-300">{{ app()->version() }}</p>
                                </div>
                                <div class="bg-pink-500/20 rounded-xl p-4 border border-pink-500/30">
                                    <p class="text-xs text-gray-400 font-semibold">PHP</p>
                                    <p class="text-lg font-bold text-pink-300">{{ PHP_VERSION }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
